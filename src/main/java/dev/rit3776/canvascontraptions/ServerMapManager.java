...
public class ServerMapManager {
    ...
    private static synchronized void initialize(ServerLevel level) {
        ...
        java.nio.file.Path dataFolder = level.getServer()
                .getWorldPath(net.minecraft.world.level.storage.LevelResource.ROOT).resolve("data");
        if (java.nio.file.Files.exists(dataFolder)) {
            try (java.util.stream.Stream<java.nio.file.Path> stream = java.nio.file.Files.list(dataFolder)) {
                stream.filter(path -> path.getFileName().toString().startsWith("map_")
        ...
        CanvasContraptions.LOGGER.info("Scan complete. Indexed {} maps in {}ms", hashToId.size(),
